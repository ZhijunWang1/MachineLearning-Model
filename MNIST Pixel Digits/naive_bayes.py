# credit: starter code provided by professor Roger Grosse
...
def train_mle_estimator(train_images, train_labels):
    ...
    # from part a, mle for theta is calculated multiple of label and training matrix devided by the total sum of each column.
    total_times = np.sum(train_labels, axis=0)
    # from part a, mle for theta is calculated multiple of label
    # and training matrix divided by the total sum of each column.
    weighted = np.dot(train_images.transpose(), train_labels)
    theta_mle = weighted * 1.0 / total_times
    # whereas, pi is probability of each category.
    pi_mle = total_times * 1.0 / sum(total_times)
...
def train_map_estimator(train_images, train_labels):
    ...
    total_times = np.sum(train_labels, axis=0)
    weighted = np.dot(train_images.transpose(), train_labels)
    # by expression from e part
    theta_map = (2.0 + weighted * 1.0) / (4.0 + total_times)
    # pi_map us the same as pi_mle
    pi_map = total_times * 1.0 / sum(total_times)
...
def log_likelihood(images, theta, pi):
    ...
    Note that l bog likelihood is not only for c^(i), it is for all possible c's."""

    # use result from b
    product1 = np.dot(images, np.log(theta))
    product2 = np.dot(1.0 - images, np.log(1-theta))
    # 0 weight for not needed log value
    part1 = np.log(pi) + product1 + product2
    part2 = logsumexp(part1)
...
def predict(log_like):
    ...
    # first filter mle
    maxi = log_like.max(axis=1, keepdims=1)
    predictions = maxi == log_like
    return predictions
